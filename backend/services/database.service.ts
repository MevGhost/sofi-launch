...
class DatabaseService {
  ...
  // User operations
  async findOrCreateUser(address: string, data?: Partial<User>) {
    if (!address) {
  ...
  async createEscrow(data: {
    ...
    if (data.verifierAddresses) {
      escrowData.verifiers = {
        create: data.verifierAddresses.map((address: string) => ({
  ...
  async getEscrowByAddress(contractAddress: string, chain: string = 'base-sepolia') {
    ...
    if (isSolanaChain && isSolanaAddress) {
      ...
      const hash = ethers.keccak256(ethers.toUtf8Bytes(contractAddress));
  ...
  async getUserEscrows(userAddress: string, role: 'project' | 'kol' | 'verifier', chain?: string) {
    // Only lowercase EVM addresses, keep Solana addresses as-is
    ...
    const whereClause: any = role === 'project' 
  ...
  async updateEscrowStatus(contractAddress: string, status: 'ACTIVE' | 'COMPLETED' | 'DISPUTED' | 'CANCELLED') {
  ...
  async updateMilestoneStatus(escrowId: string, milestoneIndex: number, data: {
    released?: boolean;
    verified?: boolean;
    releasedAt?: Date;
    verifiedAt?: Date;
  ...
  async updateSettings(id: string, data: Partial<Settings>) {
