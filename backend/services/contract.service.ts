...
export class ContractService {
  ...
  async prepareDeployEscrow(dto: CreateEscrowDto): Promise<any> {
    try {
      // Validate signature
      ...
      const totalAmountWei = ethers.parseUnits(
        ...
        dto.dealBasics.tokenDecimals
      );

      const milestoneAmounts = dto.milestones.map(m =>
        ethers.parseUnits(m.amount.toString(), dto.dealBasics.tokenDecimals)
      );

      const releaseDates = dto.milestones.map(m =>
      ...
      const verificationThreshold = this.getVerificationThreshold(
        dto.verificationSettings.verificationMethod,
        verifiers.length
      );
  ...
  async deployEscrow(dto: CreateEscrowDto): Promise<TransactionResult> {
    ...
    return {
      ...
      error: undefined,
      ...prepared
    } as TransactionResult & typeof prepared;
  }
  ...
  async releasePayment(
    ...
    try {
      ...
      const factoryContract = new ethers.Contract(factoryAddress, factoryAbi, this.signer);
  ...
  async releaseAndTransferPayment(
    escrowAddress: string,
    milestoneId: number,
    callerAddress: string
  ): Promise<TransactionResult> {
  ...
  async clawbackFunds(escrowAddress: string): Promise<TransactionResult> {
    try {
  ...
  async getEscrowDetails(escrowAddress: string): Promise<any> {
    try {
      ...
      return {
        ...
        releasedAmount: ethers.formatUnits(releasedAmount, decimals),
  ...
  private getVerificationThreshold(method: string, verifierCount: number): number {
    switch (method) {
      case 'single':
        return 1;
      case 'majority':
  ...
  async deployToken(params: {
    ...
    try {
      ...
      const tx = await this.tokenFactoryContract.createToken(
        params.name,
        params.symbol,
        params.description || '',
        params.imageUrl || '',
      ...
      if (!tokenAddress) {
        ...
        tokenAddress = ethers.getAddress('0x' + ethers.keccak256(ethers.toUtf8Bytes(params.name + params.symbol + Date.now())).slice(-40));
  ...
  async getTokenData(tokenAddress: string): Promise<any> {
    try {
      ...
      const bondingCurveContract = new ethers.Contract(devBondingCurveAddress, BONDING_CURVE_ABI, this.provider);
      ...
      try {
        ...
        if (ethReserveValue && tokenReserveValue && tokenReserveValue > 0n) {
          const ethReserve = Number(ethers.formatEther(ethReserveValue));
          const tokenReserve = Number(ethers.formatUnits(tokenReserveValue, decimals));
      ...
      const ethPriceUSD = await priceService.getETHPriceUSD();
      const priceInUsd = priceInEth * ethPriceUSD;
      const totalSupplyNumber = Number(ethers.formatUnits(totalSupply, decimals));
...
export const contractService = {
  get deployEscrow() {
    return getContractService().deployEscrow.bind(getContractService());
  },
  get releasePayment() {
    return getContractService().releasePayment.bind(getContractService());
  },
  get releaseAndTransferPayment() {
    return getContractService().releaseAndTransferPayment.bind(getContractService());
  },
  get clawbackFunds() {
    return getContractService().clawbackFunds.bind(getContractService());
  },
  get getEscrowDetails() {
    return getContractService().getEscrowDetails.bind(getContractService());
  },
  // Token launchpad methods
  get deployToken() {
    return getContractService().deployToken.bind(getContractService());
  },
  get getTokenData() {
    return getContractService().getTokenData.bind(getContractService());
  },
  get buyTokens() {
    return getContractService().buyTokens.bind(getContractService());
  },
  get sellTokens() {
    return getContractService().sellTokens.bind(getContractService());
  }
};