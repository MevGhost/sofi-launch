...
export const envSchema = z.object({
  ...
  BASE_SEPOLIA_RPC_URL: z.string().url().optional().default('https://base-sepolia.drpc.org'),
  PRIVATE_KEY: z.string().regex(/^0x[a-fA-F0-9]{64}$/, 'Invalid private key format').optional(),
  
  // Smart Contracts
  TOKEN_FACTORY_ADDRESS: z.string().regex(/^0x[a-fA-F0-9]{40}$/).optional(),
  BONDING_CURVE_ADDRESS: z.string().regex(/^0x[a-fA-F0-9]{40}$/).optional(),
  GRADUATION_MANAGER_ADDRESS: z.string().regex(/^0x[a-fA-F0-9]{40}$/).optional(),
  ESCROW_FACTORY_ADDRESS: z.string().regex(/^0x[a-fA-F0-9]{40}$/).optional(),
  ...
  ADMIN_ADDRESSES: z.string().optional(), // Comma-separated list
});
 ...
 */
export function getSanitizedConfig(): Partial<EnvConfig> {
