...
app.get('/order-validation', async (req, res) => {
  try {
    const { order_number, session_id } = req.query;
    
    console.log(`[INFO] Order validation request for order: ${order_number}, session: ${session_id}`);
    
    if (!order_number || !session_id) {
      console.log('[INFO] Missing required parameters in validation request');
      return res.status(400).json({ 
        valid: false, 
        error: 'Missing required parameters: order_number and session_id are required' 
      });
    }
    
    // Look up the order in your MongoDB by Shopify order ID
    const order = await Order.findOne({ 
      shopifyOrderId: order_number,
      sessionId: session_id
    });
    
    console.log(`[INFO] Order lookup result: ${order ? 'Found' : 'Not found'}`);
    
    if (!order) {
      return res.json({ 
        valid: false,
        message: 'No matching order found with the provided order number and session ID'
      });
    }
    
    // Order exists and session ID matches
    return res.json({
      valid: true,
      order: {
