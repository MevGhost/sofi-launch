...
import { escrowCache } from '../services/cache.service';
...
function formatContractAddress(address: string, chain: string): string {
...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const deploymentResult = await contractService.deployEscrow({
        kolAddress,
        tokenAddress,
        totalAmount,
      ...
      const savedEscrow = await db.createEscrow({
      ...
      emitWebSocketEvent('escrow:created', {
        escrow: savedEscrow,
        creator: req.user!.address,
      });

      // Clear cache
      await escrowCache.clearPattern('escrows:*');
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (escrowData.chainEscrowId) {
        const existingEscrow = await db.client.escrow.findFirst({
          where: {
      ...
      const existingByAddress = await db.client.escrow.findFirst({
        where: {
      ...
      const savedEscrow = await db.createEscrow({
        ...
        milestones: escrowData.milestones ? escrowData.milestones.map((m: any) => ({
          title: m.title,
          description: m.description,
          amount: m.amount.toString(),
      ...
      // Clear cache
      escrowCache.delete('escrows:list');
...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const totalAmount = milestones.reduce((sum: number, m: any) => sum + m.amount, 0).toString();
      ...
      const savedEscrow = await db.createEscrow({
        ...
        verificationMethod: verificationSettings.verificationMethod?.toUpperCase() as any,
        disputeResolutionMethod: verificationSettings.disputeResolutionMethod?.toUpperCase() as any,
...
router.get(
  '/',
  async (req: ChainRequest, res: Response): Promise<void> => {
    try {
      ...
      if (kolAddress) filters.kolAddress = kolAddress as string;
      if (projectAddress) filters.projectAddress = projectAddress as string;

      let escrows: any[] = [];
      
      // Handle public access with filter parameter
      if (filter && userAddress) {
        const requestChain = chain || (req as ChainRequest).chain?.slug || 'base-sepolia';
        
        if (filter === 'creator') {
          escrows = await db.getUserEscrows(userAddress as string, 'project', requestChain as string);
        } else if (filter === 'kol') {
          escrows = await db.getUserEscrows(userAddress as string, 'kol', requestChain as string);
        }
        ...
        const [projectEscrows, kolEscrows] = await Promise.all([
          db.getUserEscrows(userAddress as string, 'project', requestChain as string),
          db.getUserEscrows(userAddress as string, 'kol', requestChain as string),
        ]);
      ...
      } else if ((req as any).user?.role === 'admin' && allChains === 'true') {
        console.log('[GET /escrows] Admin detected with allChains=true, fetching ALL escrows across ALL chains');
        const allEscrows = await db.client.escrow.findMany({
          include: {
            milestones: {
              include: {
                submissions: true,
        ...
        const chain = (req as ChainRequest).chain?.slug || 'base-sepolia';
        console.log('[GET /escrows] Admin detected, fetching escrows for chain:', chain);
        const chainEscrows = await db.client.escrow.findMany({
          where: { chain },
          include: {
            milestones: {
              include: {
        ...
        const [projectEscrows, kolEscrows, verifierEscrows] = await Promise.all([
          db.getUserEscrows((req as any).user.address, 'project', requestChain as string),
          db.getUserEscrows((req as any).user.address, 'kol', requestChain as string),
          db.getUserEscrows((req as any).user.address, 'verifier', requestChain as string),
        ]);
      ...
      if (kolAddress) {
        ...
        const normalizedKolAddress = (kolAddress as string).toLowerCase();
      ...
      if (projectAddress) {
        escrows = escrows.filter(e => e.projectAddress.toLowerCase() === (projectAddress as string).toLowerCase());
      }
      ...
      const response = {
        ...
        data: paginatedEscrows.map(e => ({
          ...
          milestones: e.milestones.map((m: any) => ({
            ...
            released: m.released,
            verified: m.verified,
            submissions: m.submissions?.map((s: any) => ({
              id: s.id,
      ...
      // Cache for 1 minute
      escrowCache.set(cacheKey, response, 60);
...
router.get(
  '/:id',
  authenticate,
  async (req: AuthRequest & ChainRequest, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      ...
      if (isCuid) {
        ...
        escrowData = await db.getEscrowByAddress(id, chain);
      }
      ...
      if (!isSolana && escrowData.contractAddress && escrowData.contractAddress.startsWith('0x')) {
        try {
          onChainData = await contractService.getEscrowDetails(escrowData.contractAddress);
        } catch (error: any) {
      ...
      const escrowDetails = {
        ...
        tokenSymbol: escrowData.tokenSymbol,
        tokenDecimals: escrowData.tokenDecimals,
        status: escrowData.status.toLowerCase() as 'active' | 'completed' | 'disputed',
        chainId: escrowData.chainId,
      ...
      if (isCuid) {
        escrowCache.set(`escrow:id:${id}`, escrowDetails, 30);
        escrowCache.set(`escrow:${chain}:${escrowData.contractAddress}`, escrowDetails, 30);
      } else {
        escrowCache.set(`escrow:${chain}:${id}`, escrowDetails, 30);
        escrowCache.set(`escrow:id:${escrowData.id}`, escrowDetails, 30);
      }
      ...
      if (escrow.chain === 'solana-devnet' || escrow.chain === 'solana-mainnet') {
        ...
        try {
          ...
          try {
            ...
            // Try base58 format
            const keyBytes = bs58.decode(adminPrivateKey);
          ...
          const [escrowVaultPda] = PublicKey.findProgramAddressSync(
            [Buffer.from('escrow-vault'), escrowId.toArrayLike(Buffer, 'le', 8)],
            programId
          );
        ...
        try {
          ...
          if (!skipBlockchain) {
            ...
            const { getContractService } = await import('../services/contract.service');
...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      let escrow;
      if (isCuid) {
        escrow = await db.client.escrow.findFirst({
          where: {
            id,
            kolAddress,
          },
...
router.post(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      // Create dispute
      const dispute = await db.client.dispute.create({
        data: {
          escrowId: escrow.id,
          disputeType,
      ...
      // Emit WebSocket event
      emitWebSocketEvent('escrow:disputed', {
        escrowAddress: id,
        milestoneId: milestone.milestoneIndex,
...
router.post('/sync', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (existingEscrow) {
      ...
      const escrowWithRelations = await db.getEscrowById(existingEscrow.id);
      
      // Convert BigInt fields to strings for JSON serialization
      const serializedEscrow = {
        ...escrowWithRelations,
        blockNumber: escrowWithRelations.blockNumber?.toString() || '0',
        milestones: escrowWithRelations.milestones?.map((m: any) => ({
          ...m,
    ...
    const duplicateEscrow = await db.client.escrow.findFirst({
      where: {
        chainEscrowId: escrowId.toString(),
        chain: chain,
      }
    ...
    console.log('Creating escrow with data:', JSON.stringify(escrowData, (key, value) => 
      typeof value === 'bigint' ? value.toString() : value, 2));
    
    let escrow;
    try {
      // Create escrow in database
      escrow = await db.client.escrow.create({
        data: escrowData,
      });
    ...
    if (req.body.milestones && Array.isArray(req.body.milestones)) {
      ...
      for (let i = 0; i < req.body.milestones.length; i++) {
        ...
        await db.client.milestone.create({
          data: {
            escrowId: escrow.id,
            milestoneIndex: i,
            title: milestone.title || `Milestone ${i + 1}`,
    ...
    const escrowWithMilestones = await db.getEscrowById(escrow.id);
    
    // Convert BigInt fields to strings for JSON serialization
    const serializedEscrow = {
      ...escrowWithMilestones,
      blockNumber: escrowWithMilestones.blockNumber?.toString() || '0',
      milestones: escrowWithMilestones.milestones?.map((m: any) => ({
        ...m,
        // Convert any BigInt fields in milestones if needed
      }))
    };
...
router.get('/next-id', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const chainId = chain === 'solana-devnet' ? '901' : '900';
    const count = await db.client.escrow.count({
      where: {
        chainId: {
