...
router.get('/', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Cache for 1 minute
    await cacheService.set(cacheKey, portfolio, 60);
...
router.get('/tokens', async (req: Request, res: Response) => {
  try {
    ...
    const userAddress = (req.query.address as string) || (req as any).user?.address;
 ...
 * Get user's escrows
 */
router.get('/escrows', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const userAddress = req.user!.address;
    ...
    const [escrows, total] = await Promise.all([
      ...
      db.escrow.count({ where: whereClause })
    ]);
 ...
 */
router.get('/pnl', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const userAddress = req.user!.address;
    ...
    // Calculate PnL
    const pnlByToken: Record<string, any> = {};
    let totalBought = 0;
    let totalSold = 0;
    ...
    for (const tokenId in pnlByToken) {
      ...
      if (netCost < 0) {
        ...
        const holding = await db.tokenHolder.findFirst({
          where: {
            tokenId,
            userId: req.user!.userId
          }
