...
router.get('/escrows/:id', authenticate, authorize(UserRole.ADMIN), async (req: AuthRequest & ChainRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    if (isCuid) {
      escrow = await db.getEscrowById(id);
    } else {
      escrow = await db.getEscrowByAddress(id, req.chain?.slug || 'base-sepolia');
    }
    ...
    const formattedEscrow = {
      ...
      verifiers: escrow.verifiers.map((v: any) => ({
        address: v.address,
      ...
      activities: (escrow as any).activities?.map((a: any) => ({
        id: a.id,
        action: a.action,
...
router.get('/escrows', authenticate, authorize(UserRole.ADMIN), async (req: AuthRequest & ChainRequest, res: Response): Promise<void> => {
  try {
    ...
    const formattedEscrows = escrows.map(escrow => ({
      ...
      progress: escrow.milestones.length > 0 
        ? (escrow.milestones.filter((m: any) => m.released).length / escrow.milestones.length) * 100
        : 0,
    }));
...
// Approve escrow
router.post('/escrows/:id/approve', authenticate, authorize(UserRole.ADMIN), async (req: AuthRequest & ChainRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    emitWebSocketEvent('escrow:approved', {
      escrowId: id,
...
router.post('/escrows/:id/cancel', authenticate, authorize(UserRole.ADMIN), async (req: AuthRequest & ChainRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    if (hasUnreleasedFunds) {
      ...
      clawbackResult = await contractService.clawbackFunds(escrow.contractAddress, null as any);
    ...
    emitWebSocketEvent('escrow:cancelled', {
      escrowId: id,
      cancelledBy: adminAddress,
      reason,
      fundsReturned: hasUnreleasedFunds,
...
// Resolve dispute
router.post('/escrows/:id/dispute', authenticate, authorize(UserRole.ADMIN), async (req: AuthRequest & ChainRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    // Resolve dispute
    await db.client.dispute.update({
    ...
    emitWebSocketEvent('dispute:resolved', {
      escrowId: id,
      resolution,
      resolvedBy: req.user!.address,
      notes,
...
router.get('/stats', authenticate, authorize(UserRole.ADMIN), async (req: AuthRequest & ChainRequest, res: Response): Promise<void> => {
  try {
    ...
    const [activeEscrows, escrowsForVolume, activeUsers, pendingSubmissions] = await Promise.all([
      db.client.escrow.count({ where: { status: { in: ['ACTIVE', 'PAUSED'] }, chain } }),
      db.client.escrow.findMany({ where: { chain }, select: { totalAmount: true } }),
      db.client.user.count({ where: { lastActive: { gte: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) } } }),
      db.client.milestoneSubmission.count({ where: { status: 'PENDING' } })
    ]);
    ...
    const [completedEscrows, totalEscrows, activeDisputes] = await Promise.all([
      db.client.escrow.count({ where: { status: 'COMPLETED', chain } }),
      db.client.escrow.count({ where: { chain } }),
      db.client.dispute.count({ where: { status: 'OPEN' } })
    ]);
    
    // Get review stats for the admin acting as verifier
    const reviewedSubmissions = await db.client.milestoneSubmission.findMany({
...
// Get KOL data
router.get('/kols', authenticate, authorize(UserRole.ADMIN), async (req: AuthRequest & ChainRequest, res: Response): Promise<void> => {
  try {
    const kols = await db.client.user.findMany({
      where: { role: 'kol' },
      include: {
        _count: {
          select: {
...
// Get verifier data
router.get('/verifiers', authenticate, authorize(UserRole.ADMIN), async (_req: AuthRequest, res: Response): Promise<void> => {
  try {
    const verifiers = await db.client.user.findMany({
      where: { role: 'verifier' },
      include: {
        verifications: {
          include: {
...
// Get team members
router.get('/team', authenticate, authorize(UserRole.ADMIN), async (_req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.get('/submissions', authenticate, authorize(UserRole.ADMIN), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.get('/submissions/pending', authenticate, authorize(UserRole.ADMIN), async (_req: AuthRequest, res: Response): Promise<void> => {
  try {
    const submissions = await db.client.milestoneSubmission.findMany({
...
router.get('/escrows/:id/submissions', authenticate, authorize(UserRole.ADMIN), async (req: AuthRequest & ChainRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
...
router.post('/submissions/:id/review', authenticate, authorize(UserRole.ADMIN), async (req: AuthRequest & ChainRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    const submission = await db.client.milestoneSubmission.findUnique({
      where: { id },
      include: {
        milestone: {
          include: {
    ...
    // Emit websocket event
    emitWebSocketEvent(`submission:${action}d`, {
      submissionId: id,
    ...
    if (action === 'approve' && !submission.milestone.released) {
      try {
        ...
        if (releaseResult.success && releaseResult.transactionHash) {
          ...
          const allMilestones = await db.client.milestone.findMany({
            where: { escrowId: escrow.id }
          });
          
          const allReleased = allMilestones.every(m => m.released);
          if (allReleased) {
            await db.updateEscrowStatus(escrow.contractAddress, 'COMPLETED');
            ...
            emitWebSocketEvent('escrow:completed', {
              escrowId: escrow.id,
...
// Settings endpoints
router.get('/settings', authenticate, authorize(UserRole.ADMIN), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const settings = await db.client.settings.findFirst({
...
router.put('/settings', authenticate, authorize(UserRole.ADMIN), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const settings = await db.client.settings.upsert({
      where: { userId: req.user!.userId },
      update: req.body,
      create: {
...
router.get('/submissions/:id/details', authenticate, authorize(UserRole.ADMIN), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
