...
exports.tokenMetricsService = void 0;
...
class TokenMetricsService {
    ...
    constructor() {
        ...
        this.factory = new ethers_1.ethers.Contract(FACTORY_ADDRESS, FACTORY_ABI, this.provider);
    }
    /**
    ...
    async updateTokenMetrics(tokenAddress) {
        try {
            ...
            const priceInETH = parseFloat(ethers_1.ethers.formatEther(price));
            ...
            if (token) {
                await database_service_1.db.token.update({
                    ...
                    data: {
                        ...
                        liquidity: tokenInfo ? ethers_1.ethers.formatEther(tokenInfo.realEthReserve) : '0',
     ...
     */
    async calculate24hChange(tokenAddress) {
        try {
            const now = new Date();
     ...
     */
    async calculate24hVolume(tokenAddress) {
        try {
            const now = new Date();
            const yesterday = new Date(now.getTime() - 24 * 60 * 60 * 1000);
    ...
    async updateAllTokens() {
        try {
            ...
            for (let i = 0; i < tokens.length; i += batchSize) {
                ...
                await Promise.all(batch.map(token => this.updateTokenMetrics(token.address)));
...
exports.tokenMetricsService = new TokenMetricsService();
//# sourceMappingURL=token-metrics.service.js.map