...
router.get('/', async (req, res) => {
    try {
        // Try cache first
        const cached = await cache_service_1.escrowCache.get(CACHE_KEYS.PLATFORM_STATS);
        if (cached) {
        ...
        const [totalTokens, activeTokens, graduatedTokens, totalEscrows, activeEscrows, completedEscrows, totalUsers, totalTrades, totalVolume24h, newTokensToday, activeTraders24h, totalMarketCap,] = await Promise.all([
            database_service_1.db.token.count(),
            database_service_1.db.token.count({ where: { status: 'ACTIVE' } }),
            database_service_1.db.token.count({ where: { status: 'GRADUATED' } }),
            database_service_1.db.escrow.count(),
            database_service_1.db.escrow.count({ where: { status: 'ACTIVE' } }),
            database_service_1.db.escrow.count({ where: { status: 'COMPLETED' } }),
            database_service_1.db.user.count(),
            database_service_1.db.trade.count(),
        ...
        // Cache for 5 minutes
        await cache_service_1.escrowCache.set(CACHE_KEYS.PLATFORM_STATS, stats, 300);
...
router.get('/tokens', async (req, res) => {
    try {
        ...
        // Fetch token stats
        const [newTokens, topGainers, topLosers, mostTraded, nearGraduation,] = await Promise.all([
        ...
        // Cache for 1 minute
        await cache_service_1.escrowCache.set(cacheKey, stats, 60);
        res.json({ success: true, data: stats });
    }
    catch (error) {
...
router.get('/escrows', async (req, res) => {
    try {
        ...
        // Fetch escrow stats
        const [newEscrows, completedEscrows, disputedEscrows, totalValue, releasedValue, topKOLs, topProjects,] = await Promise.all([
            ...
            database_service_1.db.escrow.groupBy({
                by: ['kolAddress'],
                _count: true,
                orderBy: {
                    _count: {
        ...
        const [kolUsers, projectUsers] = await Promise.all([
            database_service_1.db.user.findMany({
        ...
        // Cache for 5 minutes
        await cache_service_1.escrowCache.set(cacheKey, stats, 300);
        res.json({ success: true, data: stats });
    }
    catch (error) {
