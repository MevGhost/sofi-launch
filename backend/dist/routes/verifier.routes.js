...
// Get verifier stats
router.get('/stats', auth_middleware_1.authenticate, requireVerifier, async (req, res) => {
    try {
        ...
        const pendingSubmissions = await database_service_1.db.client.milestoneSubmission.findMany({
            where: {
                status: 'PENDING',
                milestone: {
                    escrow: {
...
router.get('/submissions', auth_middleware_1.authenticate, requireVerifier, async (req, res) => {
    try {
        const verifierAddress = req.user.address.toLowerCase();
...
router.get('/submissions/:id', auth_middleware_1.authenticate, requireVerifier, async (req, res) => {
    try {
        const { id } = req.params;
        const verifierAddress = req.user.address.toLowerCase();
        // Get submission with full details
        const submission = await database_service_1.db.client.milestoneSubmission.findFirst({
            where: {
                id,
                milestone: {
                    escrow: {
...
// Review submission
router.post('/submissions/:id/review', auth_middleware_1.authenticate, requireVerifier, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        (0, websocket_service_1.emitWebSocketEvent)('milestone:reviewed', {
            escrowAddress: submission.milestone.escrow.contractAddress,
            milestoneId: submission.milestone.milestoneIndex,
            submissionId: submission.id,
            decision,
        ...
        if (decision === 'approve' && !submission.milestone.released) {
            try {
                ...
                const releaseResult = await contract_service_1.contractService.releaseAndTransferPayment(submission.milestone.escrow.contractAddress, submission.milestone.milestoneIndex, verifierAddress);
...
//# sourceMappingURL=verifier.routes.js.map