...
const token_import_service_1 = require("../services/token-import.service");
...
router.get('/', async (req, res) => {
    try {
        const { limit = '50', offset = '0', sortBy = 'marketCap', search = '', status = 'all' // all, active, graduated
         } = req.query;
        ...
        const actualSortBy = sortMapping[sortBy] || 'marketCap';
        ...
        const [tokens, total] = await Promise.all([
            database_service_1.db.token.findMany({
                where: whereClause,
                orderBy: {
                    [actualSortBy]: 'desc'
                },
            ...
            database_service_1.db.token.count({ where: whereClause })
        ]);
...
router.post('/create', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const { name, symbol, description, imageUrl, twitter, telegram, website, totalSupply = '1000000000', // 1 billion default
        ...
        const existingToken = await database_service_1.db.token.findFirst({
        ...
        if (!tokenAddress || !deploymentTx) {
            ...
            const deploymentResult = await contract_service_1.contractService.deployToken({
                name,
        ...
        try {
            ...
            if (tokenInfo) {
                ...
                initialMarketCap = tokenInfo.marketCap ?
                    (parseFloat((0, viem_1.formatEther)(BigInt(tokenInfo.marketCap))) * 1).toFixed(2) :
                    '1000';
                // Convert price from wei to ETH if needed
                if (tokenInfo.price && tokenInfo.price.length > 10 && !tokenInfo.price.includes('.')) {
                    initialPrice = (0, viem_1.formatEther)(BigInt(tokenInfo.price));
                }
                else {
        ...
        const token = await database_service_1.db.token.create({
            data: {
        ...
        await database_service_1.db.tokenHolder.create({
            data: {
                userId: user.id,
                tokenId: token.id,
        ...
        (0, websocket_service_1.emitWebSocketEvent)('token:created', {
            token,
            creator: req.user.address,
        });
        // Clear token list cache
        await cache_service_1.escrowCache.clearPattern(`${CACHE_KEYS.TOKEN_LIST}:*`);
        res.json({
            success: true,
            data: token
        });
    ...
    try {
        ...
        try {
            ...
            if (token.deploymentTx && token.deploymentTx.startsWith('0x') && token.deploymentTx.length === 66) {
                onChainData = await contract_service_1.contractService.getTokenData(address);
            }
        }
        catch (error) {
        ...
        await cache_service_1.escrowCache.set(cacheKey, tokenWithChainData, 30);
 ...
 */
router.post('/buy', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        if (transactionHash) {
            ...
            result = await contract_service_1.contractService.buyTokens({
                tokenAddress,
                amount,
        ...
        const user = await database_service_1.db.getUserByAddress(req.user.address);
        if (!user) {
            return res.status(401).json({ success: false, error: 'User not found' });
        }
        // Record trade in database
        const trade = await database_service_1.db.trade.create({
            data: {
                tokenId: token.id,
                type: 'BUY',
        ...
        const existingHolder = await database_service_1.db.tokenHolder.findFirst({
            where: {
                userId: user.id,
                tokenId: token.id,
            }
        ...
        if (existingHolder) {
            ...
            await database_service_1.db.tokenHolder.update({
        ...
        // Update token stats
        const currentHoldersCount = await database_service_1.db.tokenHolder.count({
            where: {
                tokenId: token.id,
                balance: { gt: '0' }
            }
        ...
        Promise.resolve().then(() => __importStar(require('../services/token-metrics.service'))).then(({ tokenMetricsService }) => {
            tokenMetricsService.updateTokenMetrics(tokenAddress).catch(console.error);
        });
        // Emit WebSocket event
        (0, websocket_service_1.emitWebSocketEvent)('token:trade', {
            tokenAddress,
            type: 'buy',
            trader: req.user.address,
            amount,
        ...
        // Clear caches
        await cache_service_1.escrowCache.del(`${CACHE_KEYS.TOKEN_DETAIL}${tokenAddress}`);
        await cache_service_1.escrowCache.clearPattern(`${CACHE_KEYS.TOKEN_TRADES}${tokenAddress}:*`);
        res.json({
            success: true,
            data: {
                trade,
 ...
 */
router.post('/sell', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        if (transactionHash) {
            ...
            // Backend fallback (local/dev only)
            result = await contract_service_1.contractService.sellTokens({
                tokenAddress,
                tokenAmount: amount,
        ...
        // Update or create token holder entry
        let holder = await database_service_1.db.tokenHolder.findFirst({
            where: {
                userId: user.id,
                tokenId: token.id,
            }
        ...
        if (!holder) {
            ...
            holder = await database_service_1.db.tokenHolder.create({
                data: {
                    tokenId: token.id,
...
router.get('/:address/chart', async (req, res) => {
    try {
        ...
        let trades = await database_service_1.db.trade.findMany({
            where: {
                token: { address },
        ...
        if (trades.length === 0) {
            try {
                ...
                const DevBondingCurveABI = await Promise.resolve().then(() => __importStar(require('../abis/DevBondingCurve.json')));
                const provider = new ethers.JsonRpcProvider(process.env['BASE_SEPOLIA_RPC_URL'] || 'https://base-sepolia.g.alchemy.com/v2/XK8ZLRP2DVEi2di5M9Yz0');
        ...
        const chartData = processTradesIntoChartData(trades, interval);
        ...
        await cache_service_1.escrowCache.set(cacheKey, chartData, cacheTTL);
...
router.get('/:address/holders', async (req, res) => {
    try {
        ...
        const holders = await database_service_1.db.tokenHolder.findMany({
...
router.get('/:address/trades', async (req, res) => {
    try {
        ...
        const total = await database_service_1.db.trade.count({
            where: { token: { address } }
        });
        res.json({
            success: true,
...
router.post('/:address/sync', async (req, res) => {
    try {
        ...
        let token = await database_service_1.db.token.findUnique({
        ...
        if (!token) {
            try {
                ...
                token = await token_import_service_1.tokenImportService.importToken(address, txHash);
            }
            catch (importError) {
                ...
                if (tokenData && tokenData.name && tokenData.symbol) {
                    ...
                    token = await database_service_1.db.token.create({
                        data: {
        ...
        if (imageData || metadata) {
            ...
            if (imageData && imageData.startsWith('data:image')) {
                ...
                const filepath = path_1.default.join(process.cwd(), 'uploads', 'tokens', filename);
            ...
            if (metadata) {
                ...
                if (metadata.twitter)
                    updateData.twitter = metadata.twitter;
                if (metadata.telegram)
                    updateData.telegram = metadata.telegram;
                if (metadata.website)
                    updateData.website = metadata.website;
            }
            // Update token in database if we have data to update
            if (Object.keys(updateData).length > 0) {
                token = await database_service_1.db.token.update({
                    where: { address: address.toLowerCase() },
                    data: updateData
                });
                console.log(`Updated token ${address} with image/metadata`);
                // Clear caches
                await cache_service_1.escrowCache.clearPattern(`${CACHE_KEYS.TOKEN_DETAIL}${address}*`);
                await cache_service_1.escrowCache.clearPattern(`${CACHE_KEYS.TOKEN_LIST}*`);
            }
        }
        // Try to fetch latest data from blockchain if available
        try {
            const updatedToken = await token_import_service_1.tokenImportService.updateTokenData(address);
            if (updatedToken) {
                token = updatedToken;
            }
        }
...
router.post('/sync-all', async (req, res) => {
    try {
        const count = await token_import_service_1.tokenImportService.importAllTokens();
        res.json({
            success: true,
            data: {
 ...
 */
router.post('/:address/sync-trades', async (req, res) => {
    try {
        const { address } = req.params;
        const { fromBlock } = req.body;
        const tradesImported = await trade_sync_service_1.tradeSyncService.syncTokenTrades(address, fromBlock);
        // Clear chart cache for this token
        await cache_service_1.escrowCache.clearPattern(`${CACHE_KEYS.TOKEN_CHART}${address}:*`);
        res.json({
            success: true,
            data: {
                tradesImported,
...
function processTradesIntoChartData(trades, interval) {
    if (trades.length === 0)
        return [];
