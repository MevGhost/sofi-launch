...
export function TradingViewLightweight({ tokenAddress, tokenSymbol, currentPrice = 0.000001, marketCap = 4512 }: TradingViewChartProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Create area series using the v5 API - addSeries with constructor
      try {
        ...
        console.error('Error creating area series:', err);
        ...
        try {
          ...
          console.error('Error creating line series:', err2);
        }
      }
      
      // Create histogram series for volume using v5 API
      try {
        ...
        console.error('Error creating volume series:', err);
      }
  ...
  useEffect(() => {
    ...
    const initializeData = async () => {
      ...
      // Set data on the series if it exists
      if (lineSeriesRef.current && typeof lineSeriesRef.current.setData === 'function') {
  ...
  useEffect(() => {
    if (!lineSeriesRef.current || isLoading) {
      return;
    }
    ...
    const updateChart = async () => {
      ...
      if (dataRef.current.length > 0 && dataRef.current[dataRef.current.length - 1].time === now) {
        ...
        if (lineSeriesRef.current && typeof lineSeriesRef.current.update === 'function') {
        ...
        if (volumeSeriesRef.current && typeof volumeSeriesRef.current.update === 'function') {
      ...
      dataRef.current = dataRef.current.filter(d => d.time > cutoffTime);
    };
