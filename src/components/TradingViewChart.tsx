...
export function TradingViewChart({ tokenAddress, tokenSymbol, currentPrice = 0.000001, marketCap = 4512 }: TradingViewChartProps) {
  ...
  useEffect(() => {
    ...
    let lineSeries: any;
    try {
      // Check if chart object was created properly
      if (!chart || typeof chart !== 'object') {
        console.error('Chart object not created properly');
        return;
      }
      
      // Use addLineSeries method (correct for lightweight-charts v5)
  ...
  useEffect(() => {
    if (!lineSeriesRef.current || !volumeSeriesRef.current || isLoading) return;
    ...
    const updateChart = async () => {
      ...
      if (dataRef.current.length > 0 && (dataRef.current[dataRef.current.length - 1].time as number) === now) {
  ...
  return (
    <Card className="p-6 bg-surface1 border border-border">
      ...
      <div className="relative">
        ...
        {/* Gradient overlay to simulate area chart */}
        <div 
          className="absolute inset-0 pointer-events-none"
          style={{
            background: 'linear-gradient(180deg, rgba(14, 165, 233, 0.15) 0%, rgba(14, 165, 233, 0.05) 50%, rgba(14, 165, 233, 0) 100%)',
            mixBlendMode: 'screen',
            opacity: 0.5,
          }}
        />
